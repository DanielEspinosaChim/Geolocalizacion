...
function _renderLoginScreen() {
  ...
  <div id="login-wrap" style="min-height:100vh;display:flex;align-items:center;justify-content:center;
       background:radial-gradient(ellipse at 30% 40%, #0f2660 0%, #070f1f 60%)">
    <div style="background:#0d1830;border:1px solid #1a2d56;border-radius:20px;padding:44px;width:380px;
...
_auth.onAuthStateChanged(async (fbUser) => {
  ...
  // Forzar refresh del token para que traiga custom claims actualizados
  const idTokenResult = await fbUser.getIdTokenResult(true);
  // Si no tiene custom claim de role, el backend lo busca en Firestore usuarios/{uid}
...
function _bootApp(role) {
  // Recargar la página si estábamos en el login
...
function _applyTecnicoRestrictions() {
  // Ocultar botones de eliminación y acciones destructivas
  const adminOnly = [
    'btn-eliminar-campana',
    'btn-cerrar-campana',
  ];
  ...
  // Bloquear la pestaña de predicción/zonas (solo lectura es OK, pero ocultar si se quiere)
  // Descomentar para ocultar:
  // document.querySelector('[onclick*="pred"]')?.style.setProperty('display','none');

  // Marcar visualmente el modo técnico
  console.log('[Auth] Modo técnico activo — algunas acciones están restringidas');
}
