"""
Router del módulo Canasta Básica Tracker.

Portado desde la rama `feature/colonias-reales-osm` (frontend legacy) a este
backend. Único cambio respecto al original: el acceso a Firestore. Allí venía de
`backend.core.firebase.fdb`; aquí se resuelve con el helper `db.firestore.get_db`
que ya usa el resto de routers. Se conserva el nombre `fdb` y el guard
`if fdb is None` para no tocar la lógica del compañero.
"""
import io
import json
import re
...
try:
    ...
    fdb = get_db()
except Exception:  # sin service_account.json (tests/local): degradar a 503
    fdb = None
...
@router.get(
    ...
    description=(
        ...
        "más las hojas 'Tienda y Fecha', 'Variación %' y 'Trimestres'. "
        "Parámetros opcionales: `meses` (lista separada por comas que filtra qué meses "
        "se exportan en todas las hojas) y `compare` (año contra el que se agrega la "
        "hoja de comparación '{year} vs {compare}'). "
...
# Cliente Gemini como singleton: crear el Client y re-autenticar contra el
# service account en CADA escaneo provocaba fallos de DNS intermitentes en cold
# start. Se inicializa UNA sola vez y se cachea el cliente + el módulo `types`.
_gemini_client = None
_gemini_types = None


def _get_gemini_client():
    """Devuelve (client, types) reutilizando la instancia cacheada.

    Importa google-genai de forma perezosa (puede no estar instalado en
    tests/local) e inicializa el cliente enterprise una sola vez.
    """
...
async def scan_invoice(year: str, imagen: UploadFile = File(...)):
    ...
    # Cliente Gemini singleton: se inicializa una sola vez (evita re-autenticar
    # en cada request, causa de fallos de DNS en cold start).
    try:
